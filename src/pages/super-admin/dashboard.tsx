...
export default function SuperAdminDashboard() {
  ...
  const { data: summaryResp, isLoading: loadingSummary } = useSuperAdminSummary({ preset });
  const { data: orgsResp, isLoading: loadingOrgs } = useSuperAdminOrgs({ preset });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-xl font-bold">All Organizations — Overview</h1>
        <select
          value={preset}
      ...
      <Card>
        <CardContent className="p-0">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            ...
            {loadingOrgs && <RefreshCw className="w-4 h-4 animate-spin text-muted-foreground" />}
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-muted-foreground border-b border-border">
                  ...
                  <th className="px-5 py-3 font-medium text-right">Active Campaigns</th>
              ...
              <tbody>
                ...
                {orgs.map((o) => (
                  <tr key={o.orgId} className="border-b border-border/60 hover:bg-muted/40">
